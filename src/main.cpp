...
uint256 hashGenesisBlock("0x00000152a37b4010b00bedefa9e40ac6a6f9639a57b71457a871195d3bfb838d");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GoodsCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GoodsCoin Signed Message:\n";
...
{
    // GoodsCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 3 * 60; // GoodsCoin: each block
static const int64 nTargetSpacing = 3 * 60; // GoodsCoin: 300 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GoodsCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, GoodsCoin - DarkGravity, written by Evan Duffield - evan@GoodsCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GoodsCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // GoodsCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        assert(block.hashMerkleRoot == uint256("0xc7eea500ed20582fc084af13434b4cd39c1ac0df18d2db7a2eafbf7446e3f63b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // GoodsCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GoodsCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GoodsCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GoodsCoinMiner(CWallet *pwallet)
{
    printf("GoodsCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("GoodsCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running GoodsCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GoodsCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GoodsCoinMiner, pwallet));
}
